...
export default function InteractiveSupplierRegistration() {
  ...
  const formSteps = [
    ...
    },
    {
      ...
      fields: ['business_registration_number', 'tax_pin', 'tax_compliance_number', 'tax_compliance_date'],
  ...
  const fieldHelp = {
    ...
    registration_category: "Select the category that applies to your business for preferential treatment",
    business_registration_number: "Your Certificate of Registration number from the Registrar of Companies",
    ...
    business_permit_number: "Your county/municipal business permit number (optional)",
    agpo_certificate_number: "Access to Government Procurement Opportunities certificate (optional)",
    ...
    supply_category: "Choose the category that best matches what your business supplies"
  };
  ...
  const validateCurrentStep = () => {
    ...
    currentFields.forEach(field => {
      ...
      if (field === 'agpo_certificate_number' || field === 'business_permit_number') {
        return;
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/suppliers/create.php', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-800 mb-2">Supplier Registration</h1>
          <p className="text-gray-600">Complete your registration to become an approved supplier</p>
        ...
        {formSteps[currentStep].fields.map((field) => (
          <div key={field} className="space-y-2">
            <label className="block text-sm font-medium text-gray-700">
              ...
              {field !== 'agpo_certificate_number' && field !== 'business_permit_number' && (
            ...
            {field === 'registration_category' ? (
              ...
              <input
                type={field === 'email' ? 'email' : field === 'tax_compliance_date' ? 'date' : 'text'}
                name={field}
                value={formData[field as keyof FormData]}
                onChange={handleChange}
                placeholder={`Enter ${fieldLabels[field as keyof FormData].toLowerCase()}`}
                className={`w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                  errors[field as keyof FormData] ? 'border-red-500' : 'border-gray-300'
                }`}
              />
          ...
          {Object.entries(formData).map(([key, value]) => (
            value && (
              <div key={key} className="flex">
                <span className="text-gray-600 w-40 truncate font-medium">{fieldLabels[key as keyof FormData]}:</span>
